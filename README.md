...
-현재 MoveIt상 EE이동시 물리적으로 닿을 순 있으나 계산을 못해 궤적을 생성 못하는 문제 발생
-Plan은 정상이나 Execute시 1~2회 실패, 이후 대부분 성공을 보임
-1번조인트(베이스-1번링크)가 가끔 반시계방향으로 명령없이 20~30도 움직임을 보임

-특정 물체가 있을때 피해가는 알고리즘 필요, 현재 장애물을 고려안하고 궤적생성
-자세보상함수 개선 필요
-조이스틱을 이용한 EEcontrol 개발
